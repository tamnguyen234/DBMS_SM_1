...
from benchmarks.benchmark import load_managers
...
from engines.base import Block, QueryOutcome
...
def page_styles() -> None:
    st.markdown(
        ...
        /* Layout mặc định nhạy bén dẻo dai nếu không truyền class cứng */
        .block-grid:not(.grid-6):not(.grid-3):not(.grid-2) {
        ...
        .record-pill.matched-record { 
            background: #baf3ca !important; 
            border: 1px solid #71c984 !important; 
            font-weight: 800; 
            color: #0a461c !important; 
        }
        ...
        .control-panel {
            ...
            color: #ecf1ff;
            box-shadow: 0 10px 24px rgba(67, 43, 17, 0.15);
        }
        .control-panel * {
        ...
        .control-panel button {
            ...
            background: #f1d9b4 !important;
            border: none;
        }
        .control-panel .stSelectbox, .control-panel .stNumberInput {
            min-width: 200px;
        }
        </style>
        """,
...
def render_block_grid(
    ...
    note: str | None = None,
    extra_class: str = "",
    is_matched_record: Callable[[Any], bool] | None = None,
    query_stats: str | None = None,
) -> None:
    ...
    with st.container():
        ...
        if query_stats:
            st.caption(query_stats)
        if note:
            st.caption(note)

        cards: list[str] = [f"<div class='block-grid {extra_class}'>"]
        for index in selected_indices:
            ...
            if not block.records:
                ...
                for record in block.records:
                    ...
                    is_rec_matched = is_matched_record and is_matched_record(record)
                    rec_state = "matched-record" if is_rec_matched else state
                    
                    extra_class_str = f" {record_class}" if record_class else ""
                    cards.append(
                        f"<div class='record-pill {rec_state}{extra_class_str}'>{html.escape(record_formatter(record))}</div>"
                    )
            cards.append("</div>")
...
def control_panel_ui(student_id: int, semester: str | None) -> tuple[int, str | None]:
    ...
    with cols[1]:
        query_semester = st.selectbox(
            ...
            format_func=lambda x: "(Không lọc học kỳ)" if x == "" else x,
            index=3 if semester else 0,
            key="demo_panel_semester",
            label_visibility="collapsed"
        )
    with cols[2]:
        run_query = st.button("Chạy truy vấn thao tác block", use_container_width=True, key="top_run_query")
    st.markdown("</div>", unsafe_allow_html=True)
    return int(query_student_id), (query_semester or None), run_query
...
def render_demo_grid(
    ...
    is_std_matched = lambda record: isinstance(record, Student) and record.student_id == active_student_id
    is_clus_matched = lambda record: isinstance(record, ClusterEntry) and (
        (record.table == "student" and record.payload.student_id == active_student_id)
        or (record.table == "enrollment" and record.payload.student_id == active_student_id)
    )
    is_enr_matched = lambda record: isinstance(record, Enrollment) and record.student_id == active_student_id

    if should_run:
        start = perf_counter()
        heap_result: QueryOutcome[Student] = heap_manager.search_student(active_student_id)
        heap_enrollment_result: QueryOutcome[Enrollment] = heap_manager.list_enrollments(active_student_id, active_semester)
        heap_total_reads = heap_result.block_reads + heap_enrollment_result.block_reads
        query_metrics["Heap"] = {
            "student_reads": heap_result.block_reads,
            "enrollment_reads": heap_enrollment_result.block_reads,
            "total_reads": heap_total_reads,
            "elapsed_ms": (perf_counter() - start) * 1000,
        }

        start = perf_counter()
        seq_result: QueryOutcome[Student] = seq_manager.search_student(active_student_id)
        seq_enrollment_result: QueryOutcome[Enrollment] = seq_manager.list_enrollments(active_student_id, active_semester)
        seq_total_reads = seq_result.block_reads + seq_enrollment_result.block_reads
        query_metrics["Sequential"] = {
            "student_reads": seq_result.block_reads,
            "enrollment_reads": seq_enrollment_result.block_reads,
            "total_reads": seq_total_reads,
            "elapsed_ms": (perf_counter() - start) * 1000,
        }

        start = perf_counter()
        cluster_result: QueryOutcome[Student] = cluster_manager.search_student(active_student_id)
        cluster_enrollment_result: QueryOutcome[Enrollment] = cluster_manager.list_enrollments(active_student_id, active_semester)
        cluster_total_reads = cluster_result.block_reads + cluster_enrollment_result.block_reads
        query_metrics["Clustering"] = {
            "student_reads": cluster_result.block_reads,
            "enrollment_reads": cluster_enrollment_result.block_reads,
            "total_reads": cluster_total_reads,
            "elapsed_ms": (perf_counter() - start) * 1000,
        }

        heap_matched = find_matching_blocks(heap_blocks, is_std_matched)
        seq_matched = find_matching_blocks(seq_blocks, is_std_matched)
        cluster_matched = find_matching_blocks(cluster_blocks, is_clus_matched)
        ...
        start = perf_counter()
        partition_student_result: QueryOutcome[Student] = partition_manager.search_student(active_student_id)
        if active_semester:
            ...
            if active_semester in partition_blocks:
                partition_matched[active_semester] = find_matching_blocks(
                    partition_blocks[active_semester], is_enr_matched
                )
                partition_visited[active_semester] = stop_at_first_match(
                    partition_result.visited_blocks,
                    partition_matched[active_semester],
                )
            part_total_reads = partition_student_result.block_reads + partition_result.block_reads
            query_metrics["Partitioning"] = {
                "student_reads": partition_student_result.block_reads,
                "enrollment_reads": partition_result.block_reads,
                "total_reads": part_total_reads,
                "elapsed_ms": (perf_counter() - start) * 1000,
            }
        else:
            partition_all_result: QueryOutcome[Enrollment] = partition_manager.list_enrollments(active_student_id, None)
            part_total_reads = partition_student_result.block_reads + partition_all_result.block_reads
            query_metrics["Partitioning"] = {
                "student_reads": partition_student_result.block_reads,
                "enrollment_reads": partition_all_result.block_reads,
                "total_reads": part_total_reads,
                "elapsed_ms": (perf_counter() - start) * 1000,
            }

        st.session_state["demo_query_metrics"] = query_metrics
        st.session_state["demo_query_context"] = {
    ...
    with row_one[0]:
        render_block_grid(
            ...
            is_matched_record=is_std_matched if should_run else None,
            query_stats=(
                f"SV: {int(query_metrics['Heap']['student_reads'])} · DK: {int(query_metrics['Heap']['enrollment_reads'])} · Tổng: {int(query_metrics['Heap']['total_reads'])} block"
                f" · {float(query_metrics['Heap']['elapsed_ms']):.2f} ms"
                if "Heap" in query_metrics
                else None
            ),
        )
    with row_one[1]:
        render_block_grid(
            ...
            is_matched_record=is_std_matched if should_run else None,
            query_stats=(
                f"SV: {int(query_metrics['Sequential']['student_reads'])} · DK: {int(query_metrics['Sequential']['enrollment_reads'])} · Tổng: {int(query_metrics['Sequential']['total_reads'])} block"
                f" · {float(query_metrics['Sequential']['elapsed_ms']):.2f} ms"
                if "Sequential" in query_metrics
                else None
            ),
        )
    ...
    render_block_grid(
        ...
        record_classifier=lambda record: record.table if isinstance(record, ClusterEntry) else "",
        force_indices=focus_ids["cluster"] if display_mode == "focus" else None,
        note="Một block chứa xen kẽ Student và Enrollment cùng ID.",
        is_matched_record=is_clus_matched if should_run else None,
        query_stats=(
            f"SV: {int(query_metrics['Clustering']['student_reads'])} · DK: {int(query_metrics['Clustering']['enrollment_reads'])} · Tổng: {int(query_metrics['Clustering']['total_reads'])} block"
            f" · {float(query_metrics['Clustering']['elapsed_ms']):.2f} ms"
            if "Clustering" in query_metrics
            else None
        ),
    )
    ...
    if "Partitioning" in query_metrics:
        st.caption(
            f"SV: {int(query_metrics['Partitioning']['student_reads'])} · DK: {int(query_metrics['Partitioning']['enrollment_reads'])} · Tổng: {int(query_metrics['Partitioning']['total_reads'])} block"
            f" · {float(query_metrics['Partitioning']['elapsed_ms']):.2f} ms"
        )
    st.markdown(
    ...
    for i, (semester_name, blocks) in enumerate(partition_items):
        with part_cols[i]:
            render_block_grid(
                ...
                wrap_panel=False,
                extra_class="grid-2",
                is_matched_record=is_enr_matched if (should_run and active_semester == semester_name) else None,
            )
    st.markdown("</div>", unsafe_allow_html=True)
...
def render_comparison_tab(compare_student_id: int, compare_semester: str | None) -> None:
    st.markdown("<div class='panel'>", unsafe_allow_html=True)
    st.markdown("<div class='section-title'>So sánh cả 4 kiểu tổ chức file</div>", unsafe_allow_html=True)

    metrics: dict[str, dict[str, float | int]] = st.session_state.get("demo_query_metrics", {})
    context = st.session_state.get("demo_query_context", {})
    if not metrics:
        st.info("Chưa có dữ liệu so sánh. Hãy sang tab Demo trực quan và bấm 'Chạy truy vấn' ít nhất 1 lần.")
        st.markdown("</div>", unsafe_allow_html=True)
        return

    context_student_id = context.get("student_id", compare_student_id)
    context_semester = context.get("semester", compare_semester)
    st.caption(f"Student ID: {context_student_id} · Học kỳ: {context_semester if context_semester else 'không lọc'}")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.dataframe(
        [
            {
                "Phương pháp": kind,
                "SV (block)": int(metrics[kind]["student_reads"]),
                "DK (block)": int(metrics[kind]["enrollment_reads"]),
                "Tổng Block": int(metrics[kind]["total_reads"]),
                "Thời gian (ms)": round(float(metrics[kind]["elapsed_ms"]), 2),
            }
...
def main() -> None:
    ...
    # 1. Đưa Control Panel (Bảng điều khiển) lên trên cùng
    query_student_id, query_semester, _ = control_panel_ui(
        int(st.session_state.get("demo_last_student_id", 5)),
        st.session_state.get("demo_last_semester", "2024A")
    )
    
    tab_demo, tab_compare = st.tabs(["🎮 Demo trực quan", "📊 So sánh các kiểu tổ chức file"])

    with tab_demo:
        render_demo_grid(managers, query_student_id, query_semester, display_mode)

    with tab_compare:
        render_comparison_tab(query_student_id, query_semester)
